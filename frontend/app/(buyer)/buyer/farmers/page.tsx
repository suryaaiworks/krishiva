...
import { Search, MapPin, Tag, ArrowRight, ShieldCheck, Star } from "lucide-react";
...
export default function FarmersDirectoryPage() {
  ...
  const farmers = [
    { id: 1, name: "Ramesh Patel", crop: "Sugarcane", grade: "Grade A", quantity: "15 Tons", location: "Pune, Maharashtra", rating: 4.8 },
    { id: 2, name: "Suresh Deshmukh", crop: "Wheat", grade: "Lokwan", quantity: "8 Tons", location: "Nagpur, Maharashtra", rating: 4.6 },
    { id: 3, name: "Amit Shinde", crop: "Banana", grade: "Organic", quantity: "4 Tons", location: "Jalgaon, Maharashtra", rating: 4.9 },
  ];
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-1">
          ...
          <h2 className="text-2xl font-extrabold text-foreground tracking-tight">
            Search Farmers Directory
          </h2>
        </div>

        {/* Filters */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center bg-card p-4 rounded-card border border-border/60 shadow-sm">
              ...
              <Card key={f.id} className="p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:shadow-sm transition-shadow">
                ...
                <div className="flex sm:flex-col justify-between items-end gap-3 w-full sm:w-auto border-t sm:border-t-0 border-border/30 pt-3 sm:pt-0">
                  <div className="text-left sm:text-right">
                    <span className="text-[9px] font-bold text-muted-foreground uppercase block">Available Quantity</span>
                    <span className="text-base font-black text-primary">{f.quantity}</span>
                  </div>
                  
                  <div className="flex gap-2">
                    <Button 
                      onClick={() => alert(`Opening Farmer Profile: ${f.name}...`)}
                      variant="outline"
                      className="text-[10.5px] font-bold h-8 px-2.5 rounded-btn bg-card cursor-pointer"
                    >
                      View Profile
                    </Button>
                    <Button 
                      onClick={() => alert(`Calling Ramesh Patel at +91 98765 43210...`)}
                      className="text-[10.5px] font-bold h-8 px-2.5 rounded-btn cursor-pointer gap-1"
                    >
                      Contact
                    </Button>
                  </div>
