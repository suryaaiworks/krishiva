...
export default function MachineryOwnerLoginPage() {
  const router = useRouter();
  const [email, setEmail] = React.useState("owner@krishiva.ai");
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (email === "owner@krishiva.ai" && password === "123456") {
        router.push("/machinery-owner/dashboard");
      } else {
      ...
      >
        <div className="flex flex-col items-center select-none text-center">
          ...
          <h1 className="font-heading text-2xl font-black text-foreground tracking-tight">
            Krishiva <span className="text-primary">Owner Portal</span>
          </h1>
          <p className="text-xs text-muted-foreground mt-1 max-w-xs leading-relaxed">
            Manage your fleet, receive direct rental requests, and track weekly earnings.
          </p>
        </div>
        ...
        <Card className="p-6 sm:p-8 border border-border/60">
          <form onSubmit={handleLogin} className="space-y-4.5 text-xs">
            ...
            >
              {isLoading ? "Signing in..." : "Sign In to Fleet"} <ArrowRight className="h-4 w-4" />
            </Button>
          </form>
          ...
          <div className="mt-5 pt-4 border-t border-border/40 text-[10px] text-muted-foreground/80 flex items-center justify-center gap-1.5 select-none font-semibold">
            ...
            <span>Secured Partner Portal Session</span>
          </div>
        </Card>
      </motion.div>
    </div>
