...
import { 
  ...
  ShoppingCart, Users, Package, TrendingUp, CreditCard,
...
const BUYER_NAV_ITEMS = [
  ...
  { href: "/buyer/requests", label: "Purchase Requests", icon: ShoppingCart },
  { href: "/buyer/farmers", label: "Farmers", icon: Users },
  { href: "/buyer/orders", label: "Active Orders", icon: Package },
  { href: "/buyer/insights", label: "Market Insights", icon: TrendingUp },
...
export function BuyerSidebar() {
  ...
  React.useEffect(() => {
    setMounted(true);
    const saved = localStorage.getItem("buyer_sidebar_collapsed_v2");
    if (saved !== null) {
  ...
  const handleToggleCollapse = () => {
    ...
    localStorage.setItem("buyer_sidebar_collapsed_v2", String(nextState));
  };
    ...
    >
      ...
      <div className={cn("flex items-center select-none min-h-[64px] mb-4 py-4", isCollapsed ? "justify-center px-0" : "px-3 gap-2.5")}>
        ...
        {!isCollapsed && (
          ...
          >
            Krishiva <span className="text-primary">Buyer</span>
          </motion.span>
        )}
      </div>
        ...
        {BUYER_NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <motion.div
                  layoutId="buyer-sidebar-active-v2"
        ...
        <div className="flex items-center gap-3">
          ...
          {!isCollapsed && (
            <div className="text-left min-w-0 flex-1">
              <span className="font-bold text-[11px] block text-foreground truncate leading-normal">Ganesh Bhide</span>
