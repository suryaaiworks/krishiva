...
export function MainLayout({ children }: MainLayoutProps) {
  ...
  React.useEffect(() => {
    ...
    if (!token) {
      ...
      setIsGuest(localStorage.getItem("krishiva_role") === "guest");
    }
  }, []);
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <div className="sticky top-16 z-20 w-full border-b border-emerald-500/15 bg-emerald-50/80 dark:bg-emerald-950/20 backdrop-blur-sm select-none">
          <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-center gap-3.5 flex-wrap">
            {isGuest ? (
              <>
                <span className="inline-flex items-center gap-1.5 text-[10px] font-extrabold bg-amber-500/10 text-amber-600 border border-amber-500/20 px-2 py-0.5 rounded-full animate-pulse">
                  🌱 {t("Demo Farm")}
                </span>
                ...
                <span className="inline-flex items-center text-[10px] font-bold text-amber-700 dark:text-amber-400">
                  {t("Using simulated agricultural data.")}
                </span>
              </>
            ) : (
              <>
